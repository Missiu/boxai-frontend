...
import {
  genFileChartAsync,
  genMultipleChartAsync,
  genTextChartSync,
  listChartInfo,
} from '@/services/boxai/dataChartController';
...
const ChartSync: React.FC = () => {
  ...
  const getChartListData = async () => {
    try {
      ...
      const res = await listChartInfo(
        ...
        { ...formChartListData, userId: initialState?.currentUser?.id },
      );
      if (res.code === 200) {
  ...
  const mergeAndHighlight = (list1: chartType[], list2: chartType[]) => {
    ...
    const merged = [
      ...list2,
      ...list1.filter((item) => !list2.some((searchItem) => searchItem.id === item.id)),
    ];
  ...
  const genFile = async (values: any) => {
    ...
    try {
      ...
      // 调用接口
      const res = await genFileChartAsync(
        {
  ...
  const genFiles = async (values: any) => {
    try {
      ...
      setSubmitting(true);
      // 构造请求参数
      const res = await genMultipleChartAsync(
        {
  ...
  const genText = async (values: any) => {
    try {
      ...
      setSubmitting(true);
      // 构造请求参数
      const res = await genTextChartSync({
        generationName: values.generationName,
        goalDescription: values.goalDescription,
    ...
    <Layout hasSider>
      ...
      >
        ...
        <Menu
          ...
          mode="inline"
          defaultSelectedKeys={['']}
          selectedKeys={[localStorage.getItem('chartId') || '0']}
          // items={items}
        >
          {items}
        </Menu>
